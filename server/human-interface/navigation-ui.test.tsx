...
test('loading never mounts stale canonical content or context for either Project or Entity change',()=>{const r=samples[0],projectId=r.canonical_records.project.id,entityId=r.canonical_records.entity.id;for(const request of [null,{projectId,entityId:'other',response:r},{projectId:'other',entityId,response:r}]){const html=renderToStaticMarkup(<MemoryRouter><RequestView projectId={projectId} entityId={entityId} request={request}/></MemoryRouter>);assert.ok(html.includes('role="status"'));assert.doesNotMatch(html,/hi-retained-context|Page sections|data-record-type|observation-index|hi-observation-group/);assert.ok(!html.includes(r.canonical_records.entity.label));}});
test('error never mounts retained context even if a previous response is attached',()=>{const r=samples[0],projectId=r.canonical_records.project.id,entityId=r.canonical_records.entity.id;const html=renderToStaticMarkup(<MemoryRouter><RequestView projectId={projectId} entityId={entityId} request={{projectId,entityId,response:r,error:{status:503,code:'LIVE_READ_FAILURE'}}}/></MemoryRouter>);assert.ok(html.includes('role="alert"'));assert.doesNotMatch(html,/hi-retained-context|Page sections|data-record-type|observation-index|hi-observation-group/);});
