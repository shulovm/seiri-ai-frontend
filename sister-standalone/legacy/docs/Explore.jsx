...
import { canSaveSummary as canSaveSummaryByPlan } from "./plans.js";
...
export default function Explore() {
  ...
  const canSaveKakera = canSaveSummaryByPlan(planId);
        ...
        >
          ...
          >
            ...
            <button
              ...
              disabled={!canSaveKakera}
              style={{
                width: "100%",
                padding: "11px 14px",
                background: canSaveKakera ? "#e7dbcc" : "#f0ebe0",
                border: "1px solid #e0d4c5",
                borderRadius: 10,
                color: "#5a4b3f",
                fontSize: 12,
                cursor: canSaveKakera ? "pointer" : "default",
                letterSpacing: "0.06em",
              }}
            >
              {canSaveKakera ? copy.save : (isEn ? "Upgrade to save" : "保存は有料プラン")}
            </button>
          </div>
        </div>
      )}
          ...
          >
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  if (angleFree.trim()) applyAngle(angleFree.trim());
                }
              }}
