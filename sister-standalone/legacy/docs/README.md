...
- `STRIPE_SECRET_KEY`（任意。課金導線を有効化する場合に必須）
- `STRIPE_WEBHOOK_SECRET`（任意。Webhook署名検証を有効化する場合に必須）
- `STRIPE_PRICE_LIGHT` / `STRIPE_PRICE_STANDARD` / `STRIPE_PRICE_PREMIUM`（任意。各有料プランの Stripe Price ID）

### Stripe Webhook（ローカル確認）
