...
/**
 * 最初は内蔵JSONでOK。
 * 後でJSONファイル / Supabase / Firestoreへ移せる構造。
 */
...
export class SisterThoughtOS {
  ...
  /**
   * UI側から呼ぶメイン関数。
   * LLM API接続前でも動くように、まずはルールベースで実装。
   */
  async process(
    studentInput: string,
    ...
    const safeHintLevel = this.clampHintLevel(hintLevel);
    ...
    return {
      ...
      probeQuestion: this.generateSurgicalProbe(analysis, context, safeHintLevel),
      ...
      weaknessLogCandidate: {
        ...
        suspectedCause: analysis.suspectedCause,
        hiddenWeaknessPatterns: analysis.hiddenWeaknessPatterns,
        recurringRootCause: analysis.recurringRootCause,
        confidenceScore: analysis.confidenceScore,
        suggestedMicroTraining: analysis.suggestedMicroTraining,
  ...
  private async analyzeStudentLogic(
    ...
    const suggestedMicroTraining = this.buildSuggestedMicroTraining(
      context,
    ...
    if (
      normalized.length === 0 ||
      ["わからん", "わからない", "無理", "全部わからん", "全然わかんない"].includes(
        normalized
      )
    ) {
      return {
        ...
        thoughtProcess:
          "Student cannot start. Need to check prerequisite concept first.",
        confidence: 0.82,
    ...
    if (this.looksCareless(normalized)) {
      return {
        ...
        thoughtProcess:
          "Student may understand the logic but made a surface-level mistake.",
        confidence: 0.72,
    ...
    if (this.looksProcedureError(normalized)) {
      return {
        ...
        logicBugPoint: "途中の解法選択または手順がずれている",
        thoughtProcess:
          "Student selected near-correct approach but procedural sequencing is unstable.",
        confidence: 0.69,
  ...
  private generateSurgicalProbe(
    ...
    if (analysis.detectedLevel === "FRAGMENTS") {
      return `この問題で最初に使うべき条件はどこ？${
        analysis.hiddenWeaknessPatterns[0]
          ? `（ヒント: ${analysis.hiddenWeaknessPatterns[0]}）`
          : ""
      }`;
    }
    ...
    if (analysis.detectedLevel === "PROCEDURE_ERROR") {
      if (hintLevel === 1) {
        return "手順の最初の1つだけ戻ろう。最初に整理すべき既知情報は何？";
      }
      if (hintLevel === 2) {
        return "次に使う公式や規則を1つだけ選ぶなら何？その理由は？";
      }
      return "最後に計算・根拠・結論の順で3つ並べ直すと、どこがズレていた？";
    }
  ...
  private determinePriority(
    ...
    if (analysis.detectedLevel === "VERIFIED") return "LOW";
    return "MIDDLE";
  }
  ...
  private generateNextReviewAction(
    ...
    switch (analysis.detectedLevel) {
      ...
      case "PROCEDURE_ERROR":
        return `${topicName}の解法手順を3ステップでメモし、1問で再現する`;
      case "CARELESS":
  ...
  private findConceptById(id: string): ConceptNode | null {
    for (const subject of Object.keys(this.knowledgeGraph) as Subject[]) {
      const directMatch = this.knowledgeGraph[subject][id];
  ...
  private clampHintLevel(hintLevel: HintLevel): HintLevel {
    if (hintLevel <= 1) return 1;
    if (hintLevel >= 3) return 3;
    return 2;
  }
  ...
  private inferHiddenWeaknessPatterns(
    input: string,
    context: ProblemContext
  ): string[] {
    const patterns: string[] = [];
    const text = `${context.questionText} ${input}`;

    if (context.subject === "math") {
      if (/グラフ|x|y|関数|変化/.test(text)) {
        patterns.push("変化量を比で捉える部分が不安定");
      }
      if (/比例|割合|速さ|比/.test(text)) {
        patterns.push("比例感覚が弱く、数量関係の変換で止まりやすい");
      }
      if (/文章題|日本語|式/.test(text)) {
        patterns.push("文章を式に変える工程で負荷が高い");
      }
    }

    if (context.subject === "science") {
      if (/イオン|電子|電池|中和/.test(text)) {
        patterns.push("粒子視点より用語暗記に寄りやすい");
      }
      if (/実験|結果|理由|考察/.test(text)) {
        patterns.push("実験結果から因果を言語化する工程が弱い");
      }
    }

    if (/急いだ|時間|焦/.test(input)) {
      patterns.push("時間圧による手順スキップ癖");
    }
    if (/たぶん|なんとなく/.test(input)) {
      patterns.push("見通しより勘で進める傾向");
    }

    if (patterns.length === 0) {
      patterns.push("前提知識と手順の接続が不安定");
    }
    return patterns.slice(0, 4);
  }

  private detectRecurringRootCause(
    subject: Subject,
    hiddenWeaknessPatterns: string[]
  ): string | null {
    const now = Date.now();
    const candidate = hiddenWeaknessPatterns[0];
    if (!candidate) return null;

    this.recentRootCauseHistory.push({ subject, rootCause: candidate, ts: now });
    const alive = this.recentRootCauseHistory.filter(
      (x) => now - x.ts < 1000 * 60 * 60 * 24 * 14
    );
    this.recentRootCauseHistory.length = 0;
    this.recentRootCauseHistory.push(...alive);

    const same = alive.filter(
      (x) => x.subject === subject && x.rootCause === candidate
    ).length;
    return same >= 2 ? candidate : null;
  }

  private buildSuggestedMicroTraining(
    context: ProblemContext,
    hiddenWeaknessPatterns: string[]
  ): string[] {
    if (context.subject === "math") {
      if (hiddenWeaknessPatterns.some((p) => p.includes("比例感覚"))) {
        return [
          "比例・反比例を30秒で見分けるミニ問題を3問",
          "xが1増えるとyがいくつ増えるかだけ口で言う練習",
        ];
      }
      return [
        "問題文の数量をx,yに対応づけるメモを1回作る",
        "式・表・グラフの変換を1セットだけ解く",
      ];
    }
    ...
    if (context.subject === "science") {
      return [
        "電子が増えた/減ったを粒子図で1問だけ確認",
        "実験結果→理由を1文で言う練習を2回",
      ];
    }

    return ["前提知識を5分確認", "同タイプを1問だけ解き直す"];
  }
}

export const sisterOS = new SisterThoughtOS();
