      ...
      <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button className="rounded-xl border border-slate-300 bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700">1問クイズへ</button>
          </div>
        </section>
                  ...
                  <AnimatePresence mode="wait">
                    {isSelected && showResult && (
                      <motion.span
                        ...
                        initial={isCorrect ? { opacity: 0, y: -14, scale: 0.7 } : { opacity: 0, scale: 0.75 }}
                        animate={
                          isCorrect
                            ? { opacity: 1, y: -2, scale: 1.08 }
                            : { opacity: 1, scale: [1, 1.12, 1], x: [0, -2, 2, 0] }
                        }
                        transition={{ duration: 0.6, ease: "easeOut" }}
                        className={`pointer-events-none absolute left-1/2 top-1 -translate-x-1/2 text-3xl ${
                          isCorrect ? "text-cyan-500/90" : "text-rose-400/90"
                        }`}
                      >
                        {isCorrect ? "⭕️" : "❌"}
                      </motion.span>
