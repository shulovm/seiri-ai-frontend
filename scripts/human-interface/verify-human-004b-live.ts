...
try {
 ...
 console.log(JSON.stringify({checkpoint:'HUMAN-004B',result:'PASS',project_id:LIVE_PROJECT_ID,catalog_entries:4,title:project.canonical_project.title,entity_id:entity,observation_id:observation,canonical_evidence_id:evidence,canonical_evidence_count:1,claim_linked_response_bundles:0,returned_counts:result.transport.returned_counts,before_fingerprint:before.fingerprint,after_fingerprint:afterHash,no_write:true,root_files_unchanged:true,source:result.transport.source,ui_proof:false},null,2));
