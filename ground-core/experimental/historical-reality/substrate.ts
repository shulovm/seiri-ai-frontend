 ...
 */
export interface SourceAssertionPolicy {
  confidence: number;
  applicable_from: string | null;
  applicable_until: string | null;
}

export function projectSourceAssertions(data: HistoricalDataset, state: ProjectState, recordedAt: string,
  policy?: SourceAssertionPolicy): StatePatch {
  validateDataset(data);
  if (policy && (!Number.isFinite(policy.confidence) || policy.confidence < 0 || policy.confidence > 1)) throw new Error('Invalid explicit confidence');
  ...
  for (const claim of data.claims) {
    ...
    operations.push({op:'upsert',entity:'claim',entity_id:claimId,payload:{
      ...
      confidence:policy ? policy.confidence : 0.95,
      // This projection's claim concerns the inspected text at ingestion, not its historical applicability.
      applicable_from:policy ? policy.applicable_from : recordedAt,
      applicable_until:policy ? policy.applicable_until : null,
    }});
