...
export type DecisionLifecycle = 'considered'|'candidate'|'preferred'|'intended'|'planned'|'selected'|'instructed'|'executing'|'partially_executed'|'completed';
export function interpretDecisionLifecycle(text:string): DecisionLifecycle|null {
 ...
 if(/実行.*一部|一部.*実行|部分的.*実行/.test(text)&&!/予定|つもり|未確認/.test(text))return 'partially_executed';
 if(/(?:採用|実行|実施|決める|決定する).*予定|調整している/.test(text))return 'planned';
 ...
 if(/指示(?:を送った|した)|命令(?:を送った|した)|実行するよう指示/.test(text))return 'instructed';
