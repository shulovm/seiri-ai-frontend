...
export function semanticClauses(sentence:string):{span:string,sourceSentence:string}[]{
 ...
 for(let i=0;i<sentence.length;i++){
  ...
  const tail=sentence.slice(i);const conjunct=/^(?:が、|が,)/.test(tail)&&/(?:した|された|いる|ある|ない)$/.test(sentence.slice(start,i))?tail.slice(0,2):undefined;const phaseConjunction=/^と修理後/.test(tail)&&/修理前/.test(sentence.slice(start,i))?'と':undefined;const connective=phaseConjunction??conjunct??tail.match(/^(?:そのうち|一方|ただし|しかし|だが|あるが|に対して|であり、|であり,)/)?.[0];
