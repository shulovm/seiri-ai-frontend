...
export function buildInspectionPatch(state: ProjectState, recordedAt: string): StatePatch {
  ...
  return composeInspection(state, recordedAt, { ids: idsFor(state.project.id), source, reportRef: REPORT_REF, content: `Recorded researcher-authored paraphrase: ${actor.label} — instructed to ${report.object}.` });
}
